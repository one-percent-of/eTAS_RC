...
app.controller('MotionController', function ($scope, $ionicPlatform, $cordovaDeviceMotion, $deviceGyroscope, $firebaseObject, $firebaseArray) {
  ...
  $ionicPlatform.ready(function () {
      ...
      if (cnt == 0) {
        ...
        $scope.watch2.then(null, function (error) {
          ...
          if (cnt > calTime / $scope.options.frequency) {
            ...
            //rotation judge
            if (cnt - judgeTime3 > MaxQueue / 2 && !errorAngle3 && speed > 15) {
            ...
            //uturn judge
            if (cnt - judgeTime6 > MaxQueue && !errorAngle6 && speed > 13) {
            ...
            //급가속
            if (cnt - judgeTimeAcc > secondCnt * 3 && speed >= 6 && acc_cur >= 20) {
              judgeCntAcc++;
              judgeTimeAcc = cnt;
              obj.acc = judgeCntAcc;
              obj.$save().then(function (ref) {
            ...
            //급출발
            if (cnt - judgeTimeStart > secondCnt && speed <= 5 && acc_cur >= 9) {
              judgeCntStart++;
              judgeTimeStart = cnt;
            ...
            //급감속
            if (cnt - judgeTimeDcc > secondCnt * 2 && speed >= 6 && acc_cur <= -20) {
              judgeCntDcc++;
              judgeTimeDcc = cnt;
              obj.dcc = judgeCntDcc;
              obj.$save().then(function (ref) {
            ...
            //급정지
            if (cnt - judgeTimeStop > secondCnt && speed <= 5 && acc_cur <= -10) {
              judgeCntStop++;
              judgeTimeStop = cnt;
            ...
            //급진로변경 && 급앞지르기
            if (cnt - judgeTimeLSL > secondCnt * 5 && speed >= 13 && Math.abs(angularVel) >= 10 && Math.abs(angularVelFor5) <= 2) {
              if (acc_cur <= 2)
                judgeCntCC++;
            ...
            if (speed >= 10) {
              ...
              if (cnt - judgeTimeLSL > secondCnt * 3 && CntLSL >= secondCnt * 5) {
                judgeCntLSL++;
                judgeTimeLSL = cnt;
                obj.LSL = judgeCntLSL;
                obj.$save().then(function (ref) {
    ...
    $scope.stopWatching = function () {
      ...
      var averageSpeed = (speedSum / (drivingTime * 10)).toFixed(2);
      var distance = (averageSpeed * (drivingTime / 3600)).toFixed(2);
